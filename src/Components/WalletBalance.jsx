...
const WalletBalance = () => {
  ...
  useEffect(() => {
    ...
    if (token) {
      axios
        .get(`${process.env.REACT_APP_BALANCE_API_URL}`, {
          headers: {
      ...
      axios
        .get(`${process.env.REACT_APP_ADDRESS_API_URL}`, {
          headers: {
            Authorization: `Bearer ${token}`,
            "Content-Type": "application/json",
          },
        ...
        .catch((error) => {
          console.error(
            "There was an error fetching the wallet addresses!",
            error
          );
        });
    }
  ...
  return (
    <div className="wallet-balance-container pt-8 pb-8 pr-8 pl-8 mb-4 rounded-xl">
      ...
      <div className="actions-container display: flex justify-between">
        ...
        <div className="action-button display: flex items-center flex-col text-white mr-3">
          <button className="transactionBtn">
            <FontAwesomeIcon icon={faArrowUp} className="transactionBtnIcon" />
          </button>
          <span>Withdraw</span>
        </div>
