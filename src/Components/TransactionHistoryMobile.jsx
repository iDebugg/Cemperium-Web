  ...
  return (
    <div className="transaction-history-mobile h-[calc(100vh-150px)] flex flex-col">
      <div className='flex justify-between'>
        ...
        <div className="filter-button-mobile bg-white p-3 rounded-lg cursor-pointer items-center text-md" onClick={() => setFilterOpen(!filterOpen)}>
          <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" className="bi bi-filter" viewBox="0 0 16 16" style={{ marginRight: '5px' }}>
      ...
      {/* <table className='min-w-full'>
         ...
         <tbody>
             ...
             <tr className='display: flex justify-between text-slate-600' >
               <td className='text-left text-sm'>Withdrawal <br /><span className='text-xs'>17/07/2024</span></td>
               <td className='text-sm'>Successful</td>
               <td className='text-sm'>To Oxftr56..9</td>
               <td className='text-right text-sm'>0.56 ETH  <br /><span className='text-xs'>$250</span></td>
          ...
          <tbody>
            {filteredTransactions.map((transaction, index) => (
              <tr key={index} className='flex justify-between text-slate-600'>
                <td className='text-left text-sm'>{transaction.type} <br /><span className='text-xs'>{new Date(transaction.date).toLocaleString()}</span></td>
                <td className={`status text-sm ${transaction.status.toLowerCase()}`}>{transaction.status}</td>
                <td className='text-sm'>To {transaction.address}</td>
                <td className='text-right text-sm'>{transaction.amount} {transaction.asset} <br /><span className='text-xs'>${transaction.usd}</span></td>
              </tr>
            ))}
          </tbody>
        </table>
